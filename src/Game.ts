...
export class Game {
    ...
    private carriedSoldier: InjuredSoldier | null;
    ...
    private lastLeftSpawnTime: number;
    private lastRightSpawnTime: number;
    private celebrationStartTime: number | null;
    private celebrationDuration: number;
    private mourningStartTime: number | null;
    private mourningDuration: number;
    private currentSoldierProfile: SoldierProfile | null;
    private lastSoldierProfile: SoldierProfile | null;
    private animationFrameId: number | null;
    private lastPickupGrenadeSpawnTime: number;
    private pickupGrenadeSpawnInterval: number;
    private minPickupGrenadeInterval: number;
    private maxPickupGrenadeInterval: number;
    private freezeZoneEffects: Array<{
    ...
    private checkAndSpawnForSide(team: 'left' | 'right', currentTime: number): void {
    ...
    private spawnTankOnSide(team: 'left' | 'right'): boolean {
    ...
    private getSpawnTimeForSide(team: 'left' | 'right', currentTime: number): number {
        const lastSpawnTime = team === 'left' ? this.lastLeftSpawnTime : this.lastRightSpawnTime;
        const teamTanks = this.getTanksByTeam(team);
        const activeTanks = teamTanks.filter(tank => tank.health > 0);
    ...
    private addFreezeZoneEffect(center: { x: number, y: number }, radius: number): void {
    ...
    private updateFreezeZoneEffects(): void {
        if (!this.freezeZoneEffects) return;
    ...
    private renderFreezeZoneEffects(): void {
        if (!this.freezeZoneEffects) return;
    ...
    private renderCarriedSoldierHealthMeter(): void {
        if (!this.carriedSoldier || !this.player) return;
    ...
    throwFreezeGrenade(direction?: { x: number, y: number }): void {
        if (this.player && this.player.canThrowGrenade()) {
            let throwDirection: { x: number, y: number };
    ...
    private updatePickupGrenades(): void {
    ...
    private spawnPickupGrenades(): void {
        const currentTime = Date.now();
