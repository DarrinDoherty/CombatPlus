...
export class Game {
    ...
    initializePlayer() {
        ...
        this.player = new Player({ x: playerStartX, y: playerStartY }, 20, // Smaller size than tanks
    ...
    initializeInjuredSoldier() {
        ...
        this.injuredSoldier = new InjuredSoldier({ x: soldierX, y: soldierY }, 18 // Slightly smaller than player
        );
        console.log(`Injured soldier placed at (${soldierX}, ${soldierY})`);
    }
    isPositionOccupied(x, y, existingTanks, minDistance) {
    ...
    resolveTankCollision(tank1, tank2) {
        ...
        if (overlap > 0) {
            ...
            if (tank1.angle < 0)
                tank1.angle += 2 * Math.PI;
            if (tank2.angle < 0)
                tank2.angle += 2 * Math.PI;
        }
    }
    setupEventListeners() {
    ...
    checkCollisions() {
        ...
        this.bullets = this.bullets.filter(bullet => bullet && !bullet.isOutOfBounds(this.config.canvasWidth, this.config.canvasHeight));
    }
    checkRescueMission() {
        if (!this.player || !this.injuredSoldier)
            return;
        try {
...
//# sourceMappingURL=Game_old.js.map