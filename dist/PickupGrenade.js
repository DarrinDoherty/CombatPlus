export class PickupGrenade {
    ...
    render(ctx) {
        ...
        ctx.fillRect(this.position.x - 2, this.position.y - this.size - 3, 4, 3);
        // Draw highlight
    ...
    isInPickupRange(playerPosition, playerSize) {
        const distance = Math.sqrt(Math.pow(this.position.x - playerPosition.x, 2) +
            Math.pow(this.position.y - playerPosition.y, 2));
        return distance < (this.size + playerSize / 2);
    }
}
//# sourceMappingURL=PickupGrenade.js.map